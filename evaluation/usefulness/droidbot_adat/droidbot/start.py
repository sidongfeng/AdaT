...
def parse_args():
    ...
    parser.add_argument("-init_model", action="store", dest="init_model", default='droidbot/model/checkpoint/pytorch_model.bin.20',
                        help="directory of binary classifer model path")
    parser.add_argument("-model_name", action="store", dest="model_name", default='resnet18',
                        help="name of the binary classifer model")
    parser.add_argument("-max_model_tries", action="store", dest="max_model_tries", default=input_manager.DEFAULT_MAX_MODEL_TRIES, type=int,
                        help="maximum tries for binary classifer. Default: %d" % input_manager.DEFAULT_MAX_MODEL_TRIES)
    parser.add_argument("-is_binary_classifer", action="store_false", dest="is_binary_classifer",
                        help="Declare whether use binary classifer to throttle event.")
...
def main():
    ...
    for key in sorted(opts.__dict__):
        print("  <<< {}: {}".format(key, opts.__dict__[key]))
    import os
    ...
    if start_mode == "master":
        ...
        droidbot = DroidBot(
            ...
            ignore_ad=opts.ignore_ad,
            replay_output=opts.replay_output,
            init_model=opts.init_model,
            model_name=opts.model_name,
            max_model_tries=opts.max_model_tries,
            is_binary_classifer=opts.is_binary_classifer)
        droidbot.start()
    return
