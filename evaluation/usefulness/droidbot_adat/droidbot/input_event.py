...
class EventLog(object):
    ...
    def __init__(self, device, app, event, profiling_method=None, tag=None, classifer=None, max_model_tries=4):
        self.device = device
        self.app = app
        self.event = event
        if tag is None:
        ...
        self.binary_classifer = classifer
    ...
    def save_screenshots(self, img_path, output_dir=None):
        # Save event
        if output_dir is None:
            if self.device.output_dir is None:
                return
            else:
                output_dir = os.path.join(self.device.output_dir, "clean_states")
        try:
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            to_ = "%s/%s" % (output_dir, img_path.split('/')[-1])
            shutil.copyfile(img_path, to_)
        except Exception as e:
            self.device.logger.warning("Saving clean screenshots to dir failed.")
            self.device.logger.warning(e)
    
    def save_activities(self, activities, output_dir=None):
        try:
            if output_dir is None:
                if self.device.output_dir is None:
                    return
                else:
                    output_path = os.path.join(self.device.output_dir, "activities.txt")
            if len(activities) != 0:
                activity_file = open(output_path, "a")
                activity_file.write('\n'.join(activities))
                activity_file.close()
        except Exception as e:
            self.device.logger.warning(e)
    ...
    def start(self):
        ...
        start sending event
        """
        self.from_state = self.device.get_current_state(take_screenshot=False)
    ...
    def stop(self):
        ...
        """
        self.stop_profiling()
        # Check if screenshot is fully rendered
        if self.binary_classifer is not None:
            is_blurred = True
            predict_tries = 0
            while is_blurred and predict_tries < self.max_model_tries:
                self.to_state = self.device.get_current_state(take_screenshot=True)
                # last_know_screenshot = self.device.last_know_screenshot
                # last_know_screenshot = np.array(bytearray(last_know_screenshot))
                # last_know_screenshot = cv2.imdecode(last_know_screenshot, 1)
                # if img is None:
                #     break
                is_blurred = self.binary_classifer.predict(self.to_state.screenshot_path)
                predict_tries += 1
        else:
            self.to_state = self.device.get_current_state(take_screenshot=True)
        self.save2dir()
        self.save_views()
        self.save_screenshots(self.to_state.screenshot_path)
        self.save_activities(self.to_state.activity_stack)
