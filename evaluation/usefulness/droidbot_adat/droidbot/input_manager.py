...
class InputManager(object):
    ...
    def __init__(self, device, app, policy_name, random_input,
                 ...
                 replay_output=None, binary_classifer=None, max_model_tries=None):
        """
        ...
        self.binary_classifer = binary_classifer
    ...
    def add_event(self, event):
        ...
        event_log = EventLog(self.device, self.app, event, self.profiling_method, 
                                classifer=self.binary_classifer, max_model_tries=self.max_model_tries)
        event_log.start()
        if self.binary_classifer:
            while True:
                if not self.device.pause_sending_event:
                    break
        else:
            while True:
                print('*'*20, 'sleep', self.event_interval,'*'*20)
